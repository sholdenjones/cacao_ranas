...
create pca variable based off CC and hoja b/c highly correlated
- probably just end up using tran_mean_hoja but still keep jic
```{r}
...
use glmer package to build multiple regression models
- allows for inclusion of both random and rixed effects 
## USE CC INSTEAD OF HOJA! NO DEVIATION DETECTED!!!!!!!!
```{r}
############# NO QUANTILE DEVIATION DETECTED ############
# full model with glmer, CC, veg as R.E., poisson
modfull_abundance <- glmer(num_ind ~ tran_mean_CC + 
                   water_dist_scaled +
                   forest_cover_400_scaled +
...
summary(modfull_abundance) # tran_mean_CC_sig
vif(modfull_abundance) # vif not a concern
check_overdispersion(modfull_abundance) # HERE NOT OVERDISPERSED

# assess model fit using dHARMA - NO QUANTILE DEVIATION DETECTED
...
# full model with glmer, hoja, veg as R.E., poisson
modfull_abundance_2 <- glmer(num_ind ~ tran_mean_hoja + 
                   water_dist_scaled +
                   forest_cover_400_scaled +
                   (1 | tran_mean_veg_scaled) +
                   (1 | Sitio),
...
summary(modfull_abundance_2) # nothing sig (hoja v close!)
vif(modfull_abundance_2) # vif not a concern
check_overdispersion(modfull_abundance_2) # HERE NOT OVERDISPERSED

# assess model fit using dHARMA - QUANTILE DEVIATION DETECTED
simulationOutput <- simulateResiduals(fittedModel = modfull_abundance_2)
plot(simulationOutput)
...
# d. drivers of site-level species richness
use glmer package to build multiple regression models
- allows for inclusion of both random and rixed effects 
