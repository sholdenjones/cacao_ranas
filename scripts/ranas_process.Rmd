...
## HAVING SOME ISSUES HERE - MAY NEED TO DELETE THIS SECTION
```{r}
# # Step 1: Group by Sitio and Final_ID and count occurrences
# three_column <- new %>%
#   group_by(Sitio, Final_ID) %>%
#   summarise(count = n(), .groups = 'drop')
# 
# # Step 2: Reshape the data to have 'Final_ID' as rows and 'Sitio' as columns
# # pivot_wider is used to "spread" the 'Sitio' values into columns
# flipped_matrix <- three_column %>%
#   pivot_wider(names_from = Sitio, values_from = count, values_fill = 0)
# 
# # Step 3: Convert to matrix and remove the 'Final_ID' column for use in iNEXT
# flipped_matrix <- as.matrix(flipped_matrix[,-1])  # Exclude the 'Final_ID' column
# rownames(flipped_matrix) <- three_column$Final_ID  # Assign 'Final_ID' to rownames
```


# NEW_MAT - TRY THIS METHOD FOR iNEXT
- needs species as rows, sites as columns
- code from Chat GPT, ensures retains species names?
```{r}
# #Step 1: Group by Sitio and Final_ID and count occurrences
# three_column <- new %>%
#   group_by(Sitio, Final_ID) %>%
...
# 
# # Step 2: Reshape the data to have 'Final_ID' as rows and 'Sitio' as columns
# # pivot_wider spreads the Sitio values into columns
# flipped_df <- three_column %>%
#   pivot_wider(names_from = Sitio, values_from = count, values_fill = 0)
# 
# # Step 3: Check if the row count and unique Final_IDs match
# rownames_check <- length(unique(three_column$Final_ID))
# matrix_row_count <- nrow(flipped_df)
# 
# if (rownames_check == matrix_row_count) {
#   # Step 4: Convert to matrix and assign rownames correctly
#   flipped_matrix <- as.matrix(flipped_df[,-1])  # Exclude the 'Final_ID' column
#   rownames(flipped_matrix) <- flipped_df$Final_ID  # Assign 'Final_ID' to rownames
# } else {
#   stop("Mismatch between number of Final_IDs and matrix row count.")
# }
# 
# # Remove the Final_ID column (species names), if present, since it's not needed in the list of abundances
# abundance_df <- flipped_df[,-1]
# 
# # Convert the data frame to a list of species abundances (each element in the list corresponds to a site)
# abundance_list <- as.list(as.data.frame(t(abundance_df)))
# 
# # Set the names of the list elements to be the column names (site names)
# names(abundance_list) <- colnames(df)[-1]
# 
# # Print the result
# abundance_list
# 
...
# 
# 
# out <- iNEXT(abundance_list, q=c(0, 1, 2), datatype="abundance", endpoint=500)
...
# output
# filename <- "new_mat_test_processed.csv"
# filepath <- file.path(data_dir, filename)
# write_csv(flipped_df, file = filepath)
```



## DIFFERENT WAY OF TRYING TO MAKE NEW_MAT - WRONG FORMAT for iNEXT?
```{r}
three_column <- new %>%
  group_by(Sitio) %>%
  count(`Final_ID`)
...
# Sample data frame where rows represent species and columns represent sites
df <- data.frame(
  Final_ID = c("species1", "species2", "species3"),
  Site_A = c(10, 5, 0),
  Site_B = c(4, 0, 2),
  Site_C = c(1, 3, 7)
)
