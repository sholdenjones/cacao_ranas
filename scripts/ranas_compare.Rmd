...
plot species richness across landuse types using point
```{r}
# reorder factor levels
richness_by_site$Tipo <- factor(richness_by_site$Tipo,
                         c("N", "C", "V", "B"))
...
## glm species richness with pair as random effect - instead of elevation


## NOT WORKING HERE - PROBLEMS WITH NA FOR RANDOM EFFECTS IN SUMMARY
```{r}
# richness_by_type_pair_glm <- glm(no_species ~ Tipo + (1|Pair), 
#                             data = richness_by_site, 
#              family = poisson(link = "log"))
# summary(richness_by_type_pair_glm)
# 
# # here use DHARMa package to see if we're using the correct model
# # no significant problems detected!
# simulationOutput_richness_by_type <- simulateResiduals(
#   fittedModel = richness_by_type_pair_glm)
# plot(simulationOutput_richness_by_type)
```
