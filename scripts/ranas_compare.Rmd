...
load data
## weird that dfs are of different lengths, wtf? why are columns getting added randomly?
## also note that ranas is a tbl_df and new is spec_tbl_df despite being created from ranas
```{r}
...
define a consistent palet and labels to be used in multiple figures
```{r}
# palet, keep consistent across 
pal <- brewer.pal(4, "BrBG")

# site labels, use in several plots
site_labels <-  c("Shade \n (n = 12)", "Sun \n (n = 11)", 
                              "Abandoned \n (n = 4)", "Forest \n (n = 5)")
```
...
# plot species richness across landuse types using palet
plot_richness_by_site <- ggplot(richness_by_site, aes(x = Tipo, y = no_species, fill = Tipo)) +
  geom_boxplot() +
...
# boxplot
plot_richness_by_harvest <- ggplot(richness_by_site, aes(x = harvested, y = no_species)) +
  geom_boxplot() +
...
--------------------------------------------------------------------------------
## do a bit of dataframe creation here to run statistical tests
...
--------------------------------------------------------------------------------
## now run aovs across landuse types for each diversity index
...
plot_shannon <- ggplot(shannon_df, aes(x = Tipo, y = Shannon_Index, fill = Tipo)) +
  geom_boxplot() +
  scale_fill_manual(values = pal) +
  scale_x_discrete(labels = c(site_labels)) +
  theme(legend.position = "none",
...
plot_simpson <- ggplot(simpson_df, aes(x = Tipo, y = Simpson_Index, fill = Tipo)) +
  geom_boxplot() +
  scale_fill_manual(values = pal) +
...
plot_inv_simpson <- ggplot(inv_simpson_df, aes(x = Tipo, y = Inv_Simpson_Index, 
                                               fill = Tipo)) +
  geom_boxplot() +
  scale_fill_manual(values = pal) +
  scale_x_discrete(labels = c(site_labels)) +
...
## NEXT STEP - COMPARE PLANTED AND UNPLANTED
