...
## TRANSECT LEVEL - NEED TO INCLUDE TRANSECT VISITS WHEN NO IND FOUND  
...
# MODELS ARE NOT CONVERGING FOR SITE LEVEL _ ISSUES WITH YEAR AS EITHER FIXED OR RANDOM EFFECT


## COME THROUGH AND re analyze this section

## SITE LEVEL IS NOT WORKING
...
## AT SITE LEVEL SITE_MEAN_CC_SCALED AND SITE_MEAN_HOJA_SCALED ARE CORRELATED
```{r}
# would only expect local vars or landscape vars to be correlated with each other
...
## TRY RUNNING FULL MODEL BUT WITHOUT RANDOM EFFECT _ NOT SURE WHY RE NOT WORKING

## COULD ALSO TRY RUNNING MODEL WITH SCALED ELEVATION AS RE??????

```{r}
modfull_hoja <- glmer(no_species ~ site_mean_hoja_scaled +
                   site_mean_veg_scaled + 
                   water_dist_scaled + 
                   forest_cover_400_scaled,
...
summary(modfull_hoja) # veg sig neg, hoja sig neg, CC sig pos, year sig - # AIC 418.7
vif(modfull_hoja) # vif not a concern
check_overdispersion(modfull_hoja) # not overdispersed
```
...
## CONSIDER REDUCING COLLINEARITY BY REMOVING ONE OF CORRELATED TERMS
...
# full model with CC
modfull_CC <- glmer(no_species ~ site_mean_hoja_scaled +
                   site_mean_CC_scaled +
                   site_mean_veg_scaled + # veg as R.E. prevented model convergence
                   water_dist_scaled + 
                   forest_cover_400_scaled +
...
summary(modfull_CC) # veg sig neg, hoja sig neg, CC sig pos, year sig - # AIC 418.7
vif(modfull_CC) # vif not a concern
check_overdispersion(modfull_CC) # not overdispersed
```
