...
## THIS SEEMS TO BE FOR BAYESIAN FRAMEWORK ##

# function to assess model performance using LOO-CV
# assess_models_LOO <- function(data, response, fixed_effects_list, random_effect) {
#   results <- data.frame(Model = character(),
#                         LOO_CV = numeric(),
#                         stringsAsFactors = FALSE)
# 
#   for (fixed_effects in fixed_effects_list) {
#     # Construct the model formula
#     formula_str <- paste(response, "~", fixed_effects, "+ (1 |", random_effect, ")")
#     model_formula <- as.formula(formula_str)
#     
#     # Fit the model
#     model <- glmer(model_formula, data = data, family = poisson()) # note dist here
#     
#     # Extract the log-likelihood for LOO-CV
...
#     
#     # Store the results
#     results <- rbind(results, data.frame(Model = fixed_effects, 
#                                           LOO_CV = loo_result$estimates["looic"],
#                                           stringsAsFactors = FALSE))
#   }
# 
#   return(results)
...
need to address potential variable correlation before building multivariate models
...
# c. glmer
use glmer package to build multivariate models
...
modfull <- glmer(num_species ~ tran_mean_hoja_scaled + 
                   tran_mean_CC_scaled + 
                   tran_mean_veg_scaled + # veg as re prevented model convergence
                   water_dist_scaled + 
                   forest_cover_400_scaled +
                   (1 | Sitio),
...
# would be nice to find a function that can do this!
```{r}
data <- trans_richness_local
response_variable <- "num_species"
fixed_effects_list <- c("tran_mean_hoja_scaled", 
                        "tran_mean_CC_scaled", 
                        "tran_mean_veg_scaled",
                        "water_dist_scaled", 
                        "forest_cover_400_scaled")
...
# Assess models
model_comparison <- assess_models_LOO(data, response_variable, fixed_effects_list, random_effect)
